...
monitor::monitor (void) {
    ...
    for (uint32_t i = 0 ; i < deviceCount ; i++) {
        uint64_t device;
        RSMI_CALL(rsmi_dev_unique_id_get(i, &device));
...
void monitor::query(void) {
    ...
    for (uint32_t d : indexSet) {
        ...
        rsmiUtilization_t utilization;
        rsmiDevice_t dev = devices[d];
        RSMI_CALL(rsmiDeviceGetUtilizationRates(dev, &utilization));
        {
            std::stringstream ss;
        ...
        rsmiMemory_t memory;
        RSMI_CALL(rsmiDeviceGetMemoryInfo(devices[d], &memory));
        if (!queried_once[d])
        {
            std::stringstream ss;
        ...
        /* Get clock settings */
        uint32_t clock = 0;
        RSMI_CALL(rsmiDeviceGetClock(devices[d], RSMI_CLOCK_SM,
            RSMI_CLOCK_ID_CURRENT, &clock));
        {
            std::stringstream ss;
        ...
        RSMI_CALL(rsmiDeviceGetClock(devices[d], RSMI_CLOCK_MEM,
            RSMI_CLOCK_ID_CURRENT, &clock));
        {
            std::stringstream ss;
        ...
        RSMI_CALL(rsmiDeviceGetCurrentClocksThrottleReasons(devices[d],
            &reasons));
        if (reasons && rsmiClocksThrottleReasonGpuIdle) {
            std::stringstream ss;
            ss << "GPU: Device " << d << " Throttle Idle";
            std::string tmp{ss.str()};
            sample_value(tmp, 1);
        }
        if (reasons && rsmiClocksThrottleReasonHwPowerBrakeSlowdown) {
            std::stringstream ss;
            ss << "GPU: Device " << d << " Throttle Power Break Slowdown";
            std::string tmp{ss.str()};
            sample_value(tmp, 1);
        }
        if (reasons && rsmiClocksThrottleReasonHwSlowdown) {
            std::stringstream ss;
            ss << "GPU: Device " << d << " Throttle HW Break Slowdown";
            std::string tmp{ss.str()};
            sample_value(tmp, 1);
        }
        if (reasons && rsmiClocksThrottleReasonHwThermalSlowdown) {
            std::stringstream ss;
            ss << "GPU: Device " << d << " Throttle HW Thermal Slowdown";
            std::string tmp{ss.str()};
            sample_value(tmp, 1);
        }
        if (reasons && rsmiClocksThrottleReasonSwPowerCap) {
            std::stringstream ss;
            ss << "GPU: Device " << d << " Throttle SW Power Cap Slowdown";
            std::string tmp{ss.str()};
            sample_value(tmp, 1);
        }
        if (reasons && rsmiClocksThrottleReasonSwThermalSlowdown) {
            std::stringstream ss;
            ss << "GPU: Device " << d << " Throttle SW Thermal Slowdown";
            std::string tmp{ss.str()};
            sample_value(tmp, 1);
        }
        if (reasons && rsmiClocksThrottleReasonSyncBoost) {
            std::stringstream ss;
...
#if 0
        uint32_t speed;
        RSMI_CALL(rsmiDeviceGetFanSpeed_v2(devices[d], 0, &speed));
        {
            std::stringstream ss;
        ...
        RSMI_CALL(rsmiDeviceGetPcieThroughput(devices[d],
            RSMI_PCIE_UTIL_TX_BYTES, &throughput));
        {
            std::stringstream ss;
        ...
        RSMI_CALL(rsmiDeviceGetPcieThroughput(devices[d],
            RSMI_PCIE_UTIL_RX_BYTES, &throughput));
        {
            std::stringstream ss;
        ...
        uint32_t power = 0;
        RSMI_CALL(rsmiDeviceGetPowerUsage(devices[d], &power));
        {
            std::stringstream ss;
        ...
        RSMI_CALL(rsmiDeviceGetTemperature(devices[d], RSMI_TEMPERATURE_GPU,
            &temperature));
        {
            std::stringstream ss;
        ...
        if (!queried_once[d]) {
            rsmiFieldValue_t values[2];
            int valuesCount{2};
            values[0].fieldId = RSMI_FI_DEV_NVLINK_SPEED_MBPS_COMMON;
            values[1].fieldId = RSMI_FI_DEV_NVLINK_LINK_COUNT;
            RSMI_CALL(rsmiDeviceGetFieldValues(devices[d],
                valuesCount, values));
            if (values[0].rsmiReturn == RSMI_SUCCESS)
                {
                    std::stringstream ss;
            ...
            if (values[1].rsmiReturn == RSMI_SUCCESS)
                {
                    std::stringstream ss;
...
#if defined(RSMI_FI_DEV_NVLINK_THROUGHPUT_DATA_TX)
        rsmiFieldValue_t values[6];
        values[0].fieldId = RSMI_FI_DEV_NVLINK_BANDWIDTH_C0_TOTAL;
        values[1].fieldId = RSMI_FI_DEV_NVLINK_BANDWIDTH_C1_TOTAL;
        values[2].fieldId = RSMI_FI_DEV_NVLINK_THROUGHPUT_DATA_TX;
        values[3].fieldId = RSMI_FI_DEV_NVLINK_THROUGHPUT_DATA_RX;
        values[4].fieldId = RSMI_FI_DEV_NVLINK_THROUGHPUT_RAW_TX;
        values[5].fieldId = RSMI_FI_DEV_NVLINK_THROUGHPUT_RAW_RX;
...
#if defined(RSMI_FI_DEV_NVLINK_THROUGHPUT_DATA_TX)
        if (values[2].rsmiReturn == RSMI_SUCCESS)
            {
                std::stringstream ss;
        ...
        if (values[3].rsmiReturn == RSMI_SUCCESS)
            {
                std::stringstream ss;
        ...
        if (values[4].rsmiReturn == RSMI_SUCCESS)
            {
                std::stringstream ss;
        ...
        if (values[5].rsmiReturn == RSMI_SUCCESS)
            {
                std::stringstream ss;
...
/*
double monitor::convertValue(rsmiFieldValue_t &value) {
    if (value.valueType == RSMI_VALUE_TYPE_DOUBLE) {
        return value.value.dVal;
    } else if (value.valueType == RSMI_VALUE_TYPE_UNSIGNED_INT) {
        return (double)(value.value.uiVal);
    } else if (value.valueType == RSMI_VALUE_TYPE_UNSIGNED_LONG) {
        return (double)(value.value.ulVal);
    } else if (value.valueType == RSMI_VALUE_TYPE_UNSIGNED_LONG_LONG) {
        return (double)(value.value.ullVal);
    } else if (value.valueType == RSMI_VALUE_TYPE_SIGNED_LONG_LONG) {
