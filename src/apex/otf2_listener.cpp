...
namespace apex {
    ...
    void otf2_listener::write_otf2_regions(void) {
        auto region_indices = get_region_indices();
    ...
    void otf2_listener::on_exit_thread(event_data &data) {
        if (!_terminate) {
            ...
            write_otf2_regions();
            stringstream thread;
            thread << "thread " << thread_instance::get_id();
            OTF2_GlobalDefWriter_WriteString( global_def_writer, get_string_index(thread.str()), thread.str().c_str() );
            OTF2_GlobalDefWriter_WriteLocation( global_def_writer, thread_instance::get_id() /* id */,
                        get_string_index(thread.str()) /* name */,
                        OTF2_LOCATION_TYPE_CPU_THREAD,
                        get_region_indices().size() /* # events */,
                        get_string_index(empty) /* location group */ );
        }
        APEX_UNUSED(data);
        return;
    }
    ...
    bool otf2_listener::on_start(task_identifier * id) {
        if (!_terminate) {
            OTF2_EvtWriter_Enter( evt_writer, NULL, get_time(), get_region_index(id) /* region */ );
        } else {
            return false;
        }
        return true;
    ...
    void otf2_listener::on_stop(std::shared_ptr<profiler> &p) {
        if (!_terminate) {
            OTF2_EvtWriter_Leave( evt_writer, NULL, get_time(), get_region_index(p->task_id) /* region */ );
        }
        return;
    }
