...
The `-DHPX_WITH_APEX_TAG=develop` can be used to indicate a specific release version of APEX, or to use a specific GitHub branch of APEX.  We recommend using the default configured version that comes with HPX (currently `v2.7.1`) or the `develop` branch.  Additional CMake flags include:
...
For stability, most users will want to download [the most recent release](https://github.com/UO-OACISS/apex/releases) of APEX (for example, v2.7.1):

```bash
wget https://github.com/UO-OACISS/apex/archive/refs/tags/v2.7.1.tar.gz
